...
Keep transcripts with predicted FPKM greater or equal 1 in either of both conditions (hnRNPL vs. InputKG1a). Furthermore
remore transcripts with non positive log of FPKM ratio. Here called L2FC. This is possible are not all transcripts in
the list of top genes must be upregulated (L2FC >= 1).
...
## Select Ensembl Mart
ensmart <- biomaRt::useMart("ensembl",  host="www.ensembl.org", dataset="hsapiens_gene_ensembl", verbose=F) 
...
```{r logo.for.specific.tetra.3.utr.noGs, fig.width=10, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
library(cowplot)
ncol=1
...
```{r logo.for.specific.tetra.5.utr, fig.width=10, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
...
### CDS (Summary)
```{r logo.for.specific.tetra.cds, fig.width=15, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
ncol=1
method="probability"
...
### CDS (Detailed)
```{r fig.height=36, fig.width=16, echo=F}
...
```{r logo.for.specific.tetra.ncrna, fig.width=10, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
...
```{r logo.for.specific.tetra.ncrna.noGs, fig.width=20, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
ggsl.ncrna <- ggseqlogo( c(specific.matches.ncrna, list(unspecific.matches.ncrna)), ncol = ncol, method=method)
ggsl.ncrna
...
## {.tabset .tabset-fade }
<h5 id="LogosRegulatory"><a href="#back">5.2 Regulatory</a></h4>
...
```{r fig.width=48, fig.height=15, echo=F}
#ggplot() + 
#  annotate('rect', xmin = 0.5, xmax = 3.5, ymin = -0.05, ymax = 1.9, alpha = .1, col='black', fill='yellow') +
#  geom_logo(specific.matches.ncrna, stack_width = 0.90, method = "probability") + 
...
#  for(s in 1:22){
#    print(paste("    Computing matches for regulatory motif of length ",(s+15),".", sep=""))
#    specific.regulatory.matches.mrna[[sb]][[s]] <-  match_data[[mrna]][[sb]][,2] %>% 
#                                                    stringr::str_extract_all(reg.motifs[[s]], simplify = F) %>% 
#                                                    na.omit() %>% 
#                                                    unlist() %>% 
#                                                    as.vector()
#    
#    print(paste("    Computing matches for regulatory motif of length ",(s+15)," ended.", sep=""))
#  }
#  print(paste("Computing matches for unspecific tetranucleotide CCCC or TCCCC.", sep=""))
```
