...
Keep transcripts with predicted FPKM greater or equal to 1 in either of both conditions (hnRNPL vs. InputKG1a)—furthermore, remove transcripts with a non-positive log of FPKM ratio. Here called L2FC. This is possible are not all transcripts in the list of top genes must be upregulated (L2FC >= 1).
...
### All (Summary)
```{r logo.for.specific.tetra.all, fig.width=10, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
ncol=1
method="bits"
...
```
```{r logo.for.specific.tetra.all.noGs, fig.width=10, fig.height=10, echo=F}
require(ggplot2)
require(ggseqlogo)
library(cowplot)
ncol=1
